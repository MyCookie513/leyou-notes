...
```json
{
  ...
  "receiverAddress": "航头镇航头路18号传智播客 教学办公室",
  "receiverZip": "210000",
  "invoiceType": 0,
  "sourceType":2
}
...
![1536422678407](assets/1536422678407.png)
...
![1536422928680](assets/1536422928680.png)
...
![1536423697636](assets/1536423697636.png)
...
![1536425296300](assets/1536425296300.png)



这里的收货人信息肯定是当前登录用户的收货地址。所以需要根据当前登录用户去查询，目前我们在页面是写的假数据：

![1536423800947](assets/1536423800947.png)
...
然后在购物车详情的`created`钩子函数中获取购物车数据，保存到本地属性，要注意的是，我们应该在获取数据前校验用户登录状态，如果发现未登录，则直接重定向到登录页：
...
```html
<div class="sendGoods">
    <ul class="yui3-g" v-for="(c,i) in carts" :key="i">
        <li class="yui3-u-1-6">
...
项目bug：orderController中顶部requestMapper中多了一个order，处理方式：连requestmapper一起删除，
...
不过，因为不清楚用户何时会付款，因此这里在creted方法中采用循环的方式，不断请求判断是否支付成功。
