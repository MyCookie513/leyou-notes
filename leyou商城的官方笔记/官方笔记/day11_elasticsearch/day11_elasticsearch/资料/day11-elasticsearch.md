...
但是**在Elasticsearch中，即便store设置为false，也可以搜索到结果。**

原因是**Elasticsearch在创建文档索引时，会将文档中的原始数据备份，保存到一个叫做`_source`的属性中。最终我们查询数据是从`_source`中来取值，所以可以通过过滤`_source`来选择哪些要显示，哪些不显示。**
...
在上面的案例中，不仅会查询到电视，而且与小米相关的都会查询到，查询关键字分词后多个词条之间是`or`的关系。
...
> **桶（bucket）,<span style="color:red;" >阿里文档称之为存储单元</span>**

桶的作用，是按照某种方式对数据进行分组，每一组数据在ES中称为一个`桶`，  例如我们根据国籍对人划分，可以得到`中国桶`、`英国桶`，`日本桶`……或者我们**按照年龄段**对人进行划分：0~10,10~20,20~30,30~40等。
...
public class Item {
    ...
    Double price; // 价格
    String images; // 图片地址
    //TODO add getter setters and Constructor
}
```
...
- `@Document` 作用于类，标记实体类为文档对象，一般有两个属性
  - indexName：对应索引库名称
  - type：对应在索引库中的类型
  - shards：分片数量，默认5
  - replicas：副本数量，默认1
...
修改和新增是同一个接口，区分的依据就是id，这一点跟我们在页面发起PUT请求是类似的。（与solr一致）
...
先来看最基本的match query：**PS(类似于solr的QueryParser思想)**
